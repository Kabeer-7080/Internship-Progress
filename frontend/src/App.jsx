...
function App() {
  ...
  const [loggedIn, setLoggedIn] = useState(() => Boolean(getAuthToken() && read("fg-user", null)));
  ...
  useEffect(() => {
    ...
    async function loadBackendData() {
      ...
      const databaseHealthy = health.info?.database?.status === "healthy";
      setDbActive(backendOnline && databaseHealthy);
      if (health.info) {
        setDbTelemetry(health.info);
      }

      // Restore and validate an existing session after a page refresh.
      if (backendOnline && token) {
        try {
          const user = await meApi();
          if (cancelled) return;
          setCurrentUser(user);
          localStorage.setItem("fg-user", JSON.stringify(user));
          setLoggedIn(true);

          const [apiAssessments, apiTeam] = await Promise.all([
            fetchAssessmentsApi(),
            fetchTeamApi()
          ]);
          if (!cancelled) {
            if (Array.isArray(apiAssessments)) setAssessments(apiAssessments);
            if (Array.isArray(apiTeam)) setTeam(apiTeam);
          }
        } catch (e) {
          if (!cancelled) {
            setToken(null);
  ...
  async function removeAssessment(id) {
    if (dbActive && loggedIn && token) {
      try {
        ...
        notify(e?.message || "Could not delete assessment");
      }
      return;
    }
    ...
    setAssessments((x) => x.filter((a) => a.id !== id));
    setDetail(null);
    notify("Assessment deleted locally");
  }
  async function addTeamMember(data) {
    if (dbActive && loggedIn && token) {
      try {
        const member = await createTeamMemberApi(data);
        setTeam((x) => [...x, member]);
        setModal(null);
        notify("Team member added to database");
      } catch (e) {
        notify(e?.message || "Could not add team member");
      }
      return;
    }

    const member = { ...data, id: crypto.randomUUID(), status: "Invited" };
    setTeam((x) => [...x, member]);
    setModal(null);
    notify("Invitation created locally");
  }
  async function editTeamMember(id, fields) {
    if (dbActive && loggedIn && token) {
      try {
        ...
        notify(e?.message || "Could not update team member");
      }
      return;
    }

    setTeam((x) => x.map((m) => m.id === id ? { ...m, ...fields } : m));
    notify("Team member updated locally");
    setEditingMember(null);
  }
  async function removeTeamMember(id) {
    if (dbActive && loggedIn && token) {
      try {
        ...
        notify(e?.message || "Could not remove team member");
      }
      return;
    }

    setTeam((x) => x.filter((m) => m.id !== id));
    notify("Team member removed locally");
  }
    ...
    if (dbActive && loggedIn && token) {
      try {
        ...
        for (const record of records) {
          const item = await createAssessmentApi({
            subject: record.subject,
            kind: record.kind,
            amount: record.amount,
            income: record.income,
            credit_score: record.credit,
            employment: record.employment,
            channel: record.channel
          });
          created.push(item);
        }
        setAssessments((prev) => [...created.reverse(), ...prev]);
        setModal(null);
        notify(`Imported ${created.length} assessment records into database`);
      } catch (e) {
        notify(e?.message || "Batch import failed");
      }
      return;
    }

    const created = records.map((r, index) => localCalculateAssessment(r, assessments.length + index));
    setAssessments((prev) => [...created.reverse(), ...prev]);
    setModal(null);
    notify(`Imported ${created.length} assessment records locally`);
  }
  if (loadingInitial) {
