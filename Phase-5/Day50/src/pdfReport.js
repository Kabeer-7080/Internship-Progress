...
export function generateAssessmentPDF(item) {
  const printWindow = window.open("", "_blank", "width=900,height=1000");
  if (!printWindow) {
    alert("Please allow popups to generate and view the PDF report.");
    return;
  }
  const verdictColor = item.verdict === "Approved" ? "#16a34a" : item.verdict === "Flagged" ? "#d97706" : "#dc2626";
  const verdictBg = item.verdict === "Approved" ? "#f0fdf4" : item.verdict === "Flagged" ? "#fffbeb" : "#fef2f2";
  const factorsHtml = item.factors && item.factors.length > 0 ? item.factors.map(
    (f) => `
            <tr>
              ...
              <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 700; text-align: right; color: ${f.impact === "positive" ? "#16a34a" : f.impact === "negative" ? "#dc2626" : "#64748b"};">
                ${f.impact.toUpperCase()}
              </td>
            </tr>
          `
  ).join("") : `<tr><td colspan="3" style="padding: 12px; text-align: center; color: #64748b;">No specific factor breakdown recorded</td></tr>`;
  const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <div class="header">
    ...
    <div class="report-meta">
      ...
      <div><strong>Generated:</strong> ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</div>
