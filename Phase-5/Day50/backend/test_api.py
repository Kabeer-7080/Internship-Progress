...
def run_tests():
    ...
    # 1b. Standalone ML Risk Prediction (/predict)
    pred_payload = {
        "name": "Direct ML Inference Test",
        "type": "loan",
        "income": 8500,
        "amount": 20000,
        "credit_score": 740,
    ...
    assert res_pred.status_code == 200, f"ML Predict failed: {res_pred.text}"
    pred_data = res_pred.json()
    assert "score" in pred_data and "verdict" in pred_data and "factors" in pred_data
    print(f"[PASS] POST /predict -> ML Score: {pred_data['score']}, Verdict: {pred_data['verdict']}, Factors: {len(pred_data['factors'])}")
