...
# Configure CORS for local development and the deployed Railway frontend.
# CORS_ORIGINS may contain a comma-separated list of additional/override origins.
default_origins = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    "https://zonal-love-production-76af.up.railway.app",
]
cors_env = os.getenv("CORS_ORIGINS", "")
configured_origins = [o.strip().rstrip("/") for o in cors_env.split(",") if o.strip()]

if "*" in configured_origins:
    # This project uses Bearer tokens rather than cookie credentials.
    allowed_origins = ["*"]
...
def create_assessment(
    ...
    # Generate the next reference from the existing IDs so deleting a record
    # never causes a duplicate primary key on a later create.
    existing_ids = db.query(AssessmentModel.id).all()
    numeric_ids = []
    for (existing_id,) in existing_ids:
        try:
            if str(existing_id).startswith("FG-"):
                numeric_ids.append(int(str(existing_id)[3:]))
        except (TypeError, ValueError):
            continue
    next_number = max(numeric_ids, default=10482) + 1
    ref_id = f"FG-{next_number}"
