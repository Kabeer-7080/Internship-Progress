...
from fastapi import Depends, FastAPI, HTTPException, status  # type: ignore # pyrefly: ignore
from fastapi.middleware.cors import CORSMiddleware  # type: ignore # pyrefly: ignore
from fastapi.security import HTTPAuthorizationCredentials, HTTPBearer  # type: ignore # pyrefly: ignore
from jose import JWTError, jwt  # type: ignore # pyrefly: ignore
from werkzeug.security import generate_password_hash, check_password_hash  # type: ignore # pyrefly: ignore
from sqlalchemy.orm import Session  # type: ignore # pyrefly: ignore

from .database import engine, Base, get_db, create_database_if_not_exists
        ...
        if not db.query(AssessmentModel).first():
            seeded_assessments = [
                ...
                AssessmentModel(
                    ...
                    reason="High-risk device and location", user_id="usr-admin-1"
                ),
                AssessmentModel(
...
@app.get("/health")
def health():
    return {"status": "ok", "database": "connected", "model": "loaded"}
...
def create_assessment(
    ...
    score, label, factors = risk_engine.predict(payload)

    verdict = label.capitalize()
    if verdict == "Approved":
        reason = "Strong income, credit, and employment signals"
    elif verdict == "Flagged":
        reason = "Manual review recommended for this risk profile"
    else:
        reason = "Elevated risk indicators require rejection"
...
def update_assessment(
    ...
    new_subject = body.subject or body.name
    ...
    new_credit = (
        body.credit_score if body.credit_score is not None else body.credit
    )
    ...
    score, label, factors = risk_engine.predict(payload)
    item.score = score
    item.verdict = label.capitalize()
    item.reason = (
        "Strong income, credit, and employment signals"
        if item.verdict == "Approved"
        else "Manual review recommended for this risk profile"
        if item.verdict == "Flagged"
        else "Elevated risk indicators require rejection"
    )
